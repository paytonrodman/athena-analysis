...
def main(**kwargs):
    ...
    file_dict = {'high_res': ['22000','03600'], 'high_beta': ['03600'],
                'super_res': ['00280'], 'b200_super_res': ['00550','00090']}
    os.chdir(root)

    #for num in range(0,4): # looping through problem IDs
    for prob in file_dict: # looping through problem IDs
        times = file_dict[prob]
        ...
        for t in times:
            ...
            if len(times)>1 and min(times)==t:
                file_end = '_profile_short_instant.npy'
            else:
                file_end = '_profile_instant.npy'

            np.save(prob_dir + 'dens' + file_end, dens_profile)
            np.save(prob_dir + 'mom1' + file_end, mom1_profile)
            np.save(prob_dir + 'mom2' + file_end, mom2_profile)
            np.save(prob_dir + 'mom3' + file_end, mom3_profile)
            np.save(prob_dir + 'temp' + file_end, temp_profile)
            np.save(prob_dir + 'Bcc1' + file_end, Bcc1_profile)
            np.save(prob_dir + 'Bcc2' + file_end, Bcc2_profile)
            np.save(prob_dir + 'Bcc3' + file_end, Bcc3_profile)
            np.save(prob_dir + 'rot' + file_end, orbit_v_ratio_profile)
            np.save(prob_dir + 'Rey' + file_end, stress_Rey_profile)
            np.save(prob_dir + 'Max' + file_end, stress_Max_profile)
            np.save(prob_dir + 'alpha' + file_end, alpha_SS_profile)
